...
async function scrypt(password: string, salt: string, N: number, r: number, p: number, dkLen: number): Promise<number[]> {

    function SHA256(m: number[]) {
      /** @const */ var K = [
    ...
    function salsaXOR(tmp: Uint32Array, B: Uint32Array, bin: number, bout: number) {
        var j0 = tmp[0] ^ B[bin++],
            j1 = tmp[1] ^ B[bin++],
            j2 = tmp[2] ^ B[bin++],
            j3 = tmp[3] ^ B[bin++],
    ...
    function blockCopy(dst: Uint32Array, di: number, src: Uint32Array, si: number, len: number) {
        while (len--) dst[di++] = src[si++];
    }

    function blockXOR(dst: Uint32Array, di: number, src: Uint32Array, si: number, len: number) {
        while (len--) dst[di++] ^= src[si++];
    }

    function blockMix(tmp: Uint32Array, B: Uint32Array, bin: number, bout: number, r: number) {
    ...
    var B = PBKDF2_HMAC_SHA256_OneIter(stringToUTF8Bytes(password), stringToUTF8Bytes(salt), p * 128 * r);

    var xi = 0, yi = 32 * r;

    function integerify(B: { [x: string]: any; }, bi: number, r: number) {
